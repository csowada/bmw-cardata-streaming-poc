...
mqtt_client_host = os.getenv("MQTT_CLIENT_HOST")
mqtt_client_prefix = os.getenv("MQTT_CLIENT_PREFIX", "bmw-cardata/")
...
def publish_mqtt(key: str, x: dict):
    """Push current value to mqtt client"""
    mqttc.publish(mqtt_client_prefix + key.replace(".", "/"), x["value"])
    mqttc.publish(mqtt_client_prefix + "vehicle/lastUpdate", datetime.now().isoformat())

def publish_cached_values_mqtt():
    """Push cached values to mqtt client"""

    for key, data in current_data.items():
        mqttc.publish(mqtt_client_prefix + key.replace(".", "/"), data["value"])
...
def on_message(topic: str, data: dict):
    ...
    try:
        ...
        if updates:
            ...
            for formatted in updates:
                publish_mqtt(formatted["key"], formatted)
...
def on_connect():
    ...
    mqttc.publish(mqtt_client_prefix + "client", "Online")
...
def on_disconnect(reason_code: int):
    ...
    mqttc.publish(mqtt_client_prefix + "client", "Offline")
...
def on_token_refresh(token_info: dict):
    ...
    # Publish access token to MQTT, needed for API access
    mqttc.publish(mqtt_client_prefix + "access_token", token_info.get("access_token"))
...
def on_mqtt_connect(client, userdata, flags, reason_code, properties):
    ...
    mqttc.publish(mqtt_client_prefix + "lwt", "Online", 0, True)

    publish_cached_values_mqtt()
...
def on_mqtt_disconnect(client, userdata, flags, reason_code, properties):
    ...
    mqttc.publish(mqtt_client_prefix + "client", "Offline", 0, True)
    mqttc.loop_stop()
...
def main():
    ...
    # Connect to MQTT client if configurated
    if mqtt_client_host:
        mqttc.on_connect = on_mqtt_connect
        mqttc.on_disconnect = on_mqtt_disconnect

        logger.info(f"Connect to MQTT client {mqtt_client_host} ...")
        mqttc.will_set(mqtt_client_prefix + "client", "Offline", 0, True)
        mqttc.connect(mqtt_client_host, 1883, 60)
